...
@export_range(0, 150) var comm_radius = 0.0
...
func _ready():
	...
	DecisionTree = load("res://decision_architecture/decision_tree.gd")
...
func _generate_boids():
	for i in NUM_BOIDS:
		...
		var boid = Boids.new(self, data_node, tasks_node, i, pos, vel, 0, sqrt(pow(get_viewport_rect().size.y, 2) + pow(get_viewport_rect().size.x, 2)), instantiatedTree, random_integers)
		boids.append(boid)
...
func trainTree(file, tree):
	#Handle and split data in csv file
	var dataset = load_csv_data(file)
	
	if dataset.size() == 0:
		print("Dataset Load Exception!")
	#Splitting for XY where Y will be [-1]
	var X = []
	var Y = []
	for row in dataset:
		X.append(row.slice(0,4))
		Y.append(row[4])
	#Num of Features | Max Depth
	instantiatedTree = tree.new(4, 6)
	tree.fit(X,Y)
