...
func _init(main_node, data_node, tasks_node, boidID, pos, vel, group, frame, decisionTree, need = [10,15,8]) -> void:
	...
	main = main_node # Store reference to Main (Need to do this because of the way godot calls nodes and instances)
	position = pos
	velocity = vel
	groupID = group
	self.frame = frame
...
func findMotivationForGoal(position):
	max_task = "none"
	var instrumentality_features = []
	for i in given_need.keys().size():
		task = given_need.keys()[i]
		instrumentality_features.append_array(computeInstrumentality(nearest_tasks[task], task))
		#print(instrumentality, " abc " ,instrumentality_current)
	var input_data = [instrumentality_features]
	var new_gID = decisionTree.predict(input_data)[0]
...
func computeInstrumentality(est_goal_pos: Vector2, task: String):
	...
	effort = (abs(est_goal_pos - position).length()) / frame #(0,1)
